		...
		if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
		{
			...
			if (type == PointConverter.TypeValue)
			{
				...
				if (coordinatesProperty.ValueKind != JsonValueKind.Array)
					throw new JsonException($"Cannot deserialize {nameof(GeometryPoint)} because coordinates must be an array");
				...
				if (arrayLength != COORDINATES_ARRAY_LENGTH)
					throw new JsonException($"Cannot deserialize {nameof(GeometryPoint)} because it contains wrong coordinates");
			...
			throw new JsonException($"Cannot deserialize {nameof(GeometryPoint)} because of type \"{type}\"");
		}
