...
{
    public override GeometryLineString? Read(
    ...
    )
    {
        ...
        if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
        {
            ...
            if (type == LineStringConverter.TypeValue)
            {
                ...
                if (coordinatesProperty.ValueKind != JsonValueKind.Array)
                    throw new JsonException(
                        $"Cannot deserialize {nameof(GeometryLineString)} because coordinates must be an array"
                    );
            ...
            throw new JsonException(
                $"Cannot deserialize {nameof(GeometryLineString)} because of type \"{type}\""
            );
        }
