...
{
    public override GeometryMultiLineString? Read(
    ...
    )
    {
        ...
        if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
        {
            ...
            if (type == MultiLineStringConverter.TypeValue)
            {
                ...
                if (coordinatesProperty.ValueKind != JsonValueKind.Array)
                    throw new JsonException(
                        $"Cannot deserialize {nameof(GeometryMultiLineString)} because coordinates must be an array"
                    );
            ...
            throw new JsonException(
                $"Cannot deserialize {nameof(GeometryMultiLineString)} because of type \"{type}\""
            );
        }
