...
{
    public override GeographyPolygon? Read(
    ...
    )
    {
        ...
        if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
        {
            ...
            if (type == PolygonConverter.TypeValue)
            {
                ...
                if (coordinatesProperty.ValueKind != JsonValueKind.Array)
                    throw new JsonException(
                        $"Cannot deserialize {nameof(GeographyPolygon)} because coordinates must be an array"
                    );
            ...
            throw new JsonException(
                $"Cannot deserialize {nameof(GeographyPolygon)} because of type \"{type}\""
            );
        }
