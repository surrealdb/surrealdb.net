...
{
    public override GeometryMultiPolygon? Read(
    ...
    )
    {
        ...
        if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
        {
            ...
            if (type == MultiPolygonConverter.TypeValue)
            {
                ...
                if (coordinatesProperty.ValueKind != JsonValueKind.Array)
                    throw new JsonException(
                        $"Cannot deserialize {nameof(GeometryMultiPolygon)} because coordinates must be an array"
                    );
            ...
            throw new JsonException(
                $"Cannot deserialize {nameof(GeometryMultiPolygon)} because of type \"{type}\""
            );
        }
