...
internal sealed class Surreal2ExpressionVisitor : ExpressionVisitor
{
    ...
    public Surreal2ExpressionVisitor(
        ...
        if (projectionExpression is FieldsProjectionExpression fieldsProjectionExpression)
        {
            var fields = fieldsProjectionExpression
                .Fields.SelectMany(f =>
                {
                    ...
                    // if (valueExpression is not null)
                    // {
                    //     var alias = string.IsNullOrWhiteSpace(f.Alias)
                    //         ? null
                    //         : new IdiomExpression([new FieldPartExpression(f.Alias)]);
    ...
    protected override Expression VisitMember(MemberExpression node)
    {
        ...
        var (fieldName, fromAttribute) = ReflectionExtensions.GetDatabaseFieldName(node.Member);
