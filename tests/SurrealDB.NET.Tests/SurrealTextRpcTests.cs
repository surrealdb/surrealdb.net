...
public sealed class SurrealTextRpcTests
{
	private readonly SurrealDbCliFixture _fixture;

	private ISurrealRpcClient _client => _fixture.Rpc;

	public SurrealTextRpcTests(SurrealDbCliFixture fixture)
	{
		_fixture = fixture;
	}
	...
	{
		await _client.UseAsync("test", "test");
	}

	[Fact(DisplayName = "info without scope", Timeout = 5000)]
	public async Task InfoWithoutScope()
	{
		_ = await _client.SigninRootAsync("root", "root");
		_ = await _client.InfoAsync<User>();
	}
	...
	public async Task Signup()
	{
		...
		await _client.SigninRootAsync("root", "root");
		var token = await _client.SignupAsync("test", "test", "account", user);
	...
	public async Task SigninScope()
	{
		...
		var signinToken = await _client.SigninAsync("test", "test", "account", user);
	...
	public async Task Authenticate()
	{
		...
		var signupToken = await _client.SignupAsync("test", "test", "account", user);
		Assert.NotNull(signupToken);
		await _client.AuthenticateAsync(signupToken);
	}
	...
	public async Task Invalidate()
	{
		...
		Assert.NotNull(signupToken);
		await _client.AuthenticateAsync(signupToken);
		await _client.InvalidateAsync();
		user = await _client.InfoAsync<User>();
		Assert.Null(user);
	}
	...
	public async Task Let()
	{
		...
		await _client.LetAsync("xunit", value);
		var result = await _client.QueryAsync("RETURN $xunit");
	...
	public async Task Unset()
	{
		var value = Assembly.GetExecutingAssembly().GetName().Version!.ToString();
		await _client.LetAsync("xunit", value);
		var result1 = await _client.QueryAsync("RETURN $xunit");
		Assert.Equal(value, result1.Get<string>(0));
		await _client.UnsetAsync("xunit");
		var result2 = await _client.QueryAsync("RETURN $xunit");
	...
	public async Task Live()
	{
		...
		var liveId = await _client.LiveAsync<Post>("post", (post, type) =>
		{
			notification.SetResult((post, type));

		});
		Assert.NotEqual(new SurrealLiveQueryId(), liveId);
		var inserted = await _client.InsertAsync("post", new Post("SurrealDB is pretty cool"));
	...
	public async Task Kill()
	{
		...
		var live = await _client.LiveAsync<Post>("post", (post, type) =>
		{
			tcs.SetResult((post, type));

		});
		await _client.KillAsync(live);
	...
	{
		var result = await _client.QueryAsync("SELECT * FROM ONLY type::thing($id)", new
		{
			id = new Thing
			{
				Table = "post",
		...
		Assert.Throws<SurrealException>(() => result.Get<Post>());
	}

	[Fact(DisplayName = "query single non-existent record should return null", Timeout = 5000)]
	public async Task QuerySingleNotFound()
	{
		var result = await _client.QueryAsync("SELECT * FROM type::thing($id)", new
		{
			id = Thing.Parse("post:NotFound")
	...
	{
		var result = await _client.QueryAsync("SELECT * FROM post WHERE id = type::thing($id)", new
		{
			id = new Thing
			{
				Table = "post",
	...
	{
		var inserted = await _client.InsertAsync("post", new Post[]
		{
	...
	public async Task SelectOneFound()
	{
		...
		var created = await _client.CreateAsync(id, new Post(content));
		Assert.NotNull(created);
		var result = await _client.SelectAsync<Post>(id);
	...
	public async Task CreateWithManualId()
	{
		...
		var created = await _client.CreateAsync(manual, new Post("Creating with manual id"));
		Assert.NotNull(created);
	}
	...
	{
		var content = "SurrealDB is pretty cool";
		var inserted = await _client.InsertAsync("post", new Post(content));
		Assert.NotNull(inserted);
	...
	public async Task UpdateExistingRecord()
	{
		...
		var created = await _client.CreateAsync(id, post);
		var updated = await _client.UpdateAsync(id, created with
		{
			Content = "New content!"
	...
	public async Task UpdateNonExistentRecordFunctionsAsUpsert()
	{
		...
		var upserted = await _client.UpdateAsync(id, post);
	...
	public async Task MergeIntoExisting()
	{
		...
		var created = await _client.CreateAsync(id, new Post("This post will be merged later"));
		Assert.NotNull(created);
		const string tags = "surrealdb,merge,tests";
		var merged = await _client.MergeAsync<Post>(id, new
		{
			tags
	...
	public async Task MergeIntoTable()
	{
		...
		{
			var created = await _client.CreateAsync(id, new Post($"Post #{id.Id}"));
			Assert.NotNull(created);
		}

		const string tags = "surrealdb,merge,tests";
		var merged = await _client.BulkMergeAsync<Post>("post", new
		{
			tags
	...
	public async Task PatchAddTestOk()
	{
		...
		await _client.PatchAsync<Post>(id, patch => patch
			.Add(p => p.Tags, tags)
			.Test(p => p.Content, content))
			;

		var post = await _client.SelectAsync<Post>(id);
		Assert.NotNull(post);
	...
	public async Task PatchAddTestFailed()
	{
		...
		var ex = await Assert.ThrowsAnyAsync<Exception>(() => _client.PatchAsync<Post>(id, patch => patch
			.Add(p => p.Tags, tags)
	...
	public async Task PatchMoveTestOk()
	{
		...
		var created = await _client.CreateAsync(id, new Post(content) { Tags = tags });
		Assert.NotNull(created);
	...
	public async Task DeleteExistingRecord()
	{
		...
		var existing = await _client.CreateAsync(id, new Post("This will be deleted"));

		var deleted = await _client.DeleteAsync<Post>(id);
