...
public class RelateTests
{
    ...
    )
    {
        ...
        Func<Task> func = async () =>
        {
            ...
            var complexOne = await client.Create<OutputRecord, OutputRecord>(
                (StringRecordId)"complex:one"
            );
            var complexTwo = await client.Create<OutputRecord, OutputRecord>(
                (StringRecordId)"complex:two"
            );
