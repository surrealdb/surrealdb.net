...
public class ParametersQueryableTests : BaseQueryableTests
{
    ...
    public Task ObjectParameter()
    {
        ...
        query
            ...
            .Be(
                """
                SELECT City, Country, id, IsActive, Number, State, Street, ZipCode FROM address WHERE IsActive && Country == $search.Country && City == $search.City
                """
            );
        return Verify(Parameters, _verifySettings);
    }
    ...
    public Task NullParameter()
    {
        ...
        query
            ...
            .Be(
                """
                SELECT content, created_at, id, status, title FROM post WHERE status == $status
                """
            );
        return Verify(Parameters, _verifySettings);
    }
    ...
    public Task ReservedParameterNames()
    {
        ...
        query
            ...
            .Be(
                """
                SELECT City, Country, id, IsActive, Number, State, Street, ZipCode FROM address WHERE Country == $_access || Country == $_auth || Country == $_token || Country == $_session
                """
            );
        return Verify(Parameters, _verifySettings);
    }
