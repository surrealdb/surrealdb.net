...
{
	public override GeographyMultiPolygon? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
		{
			...
			if (type == MultiPolygonConverter.TypeValue)
			{
				...
				if (coordinatesProperty.ValueKind != JsonValueKind.Array)
					throw new JsonException($"Cannot deserialize {nameof(GeographyMultiPolygon)} because coordinates must be an array");
			...
			throw new JsonException($"Cannot deserialize {nameof(GeographyMultiPolygon)} because of type \"{type}\"");
		}

		throw new JsonException($"Cannot deserialize {nameof(GeographyMultiPolygon)}");
	}

	public override void Write(Utf8JsonWriter writer, GeographyMultiPolygon value, JsonSerializerOptions options)
	{
