...
{
	public override GeometryPolygon? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
		{
			...
			if (type == PolygonConverter.TypeValue)
			{
				...
				if (coordinatesProperty.ValueKind != JsonValueKind.Array)
					throw new JsonException($"Cannot deserialize {nameof(GeometryPolygon)} because coordinates must be an array");
			...
			throw new JsonException($"Cannot deserialize {nameof(GeometryPolygon)} because of type \"{type}\"");
		}

		throw new JsonException($"Cannot deserialize {nameof(GeometryPolygon)}");
	}

	public override void Write(Utf8JsonWriter writer, GeometryPolygon value, JsonSerializerOptions options)
	{
