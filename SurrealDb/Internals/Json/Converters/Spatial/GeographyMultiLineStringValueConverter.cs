...
{
	public override GeographyMultiLineString? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
	{
		...
		if (root.TryGetProperty(SpatialConverterConstants.TypePropertyName, out var typeProperty))
		{
			...
			if (type == MultiLineStringConverter.TypeValue)
			{
				...
				if (coordinatesProperty.ValueKind != JsonValueKind.Array)
					throw new JsonException($"Cannot deserialize {nameof(GeographyMultiLineString)} because coordinates must be an array");
			...
			throw new JsonException($"Cannot deserialize {nameof(GeographyMultiLineString)} because of type \"{type}\"");
		}

		throw new JsonException($"Cannot deserialize {nameof(GeographyMultiLineString)}");
	}

	public override void Write(Utf8JsonWriter writer, GeographyMultiLineString value, JsonSerializerOptions options)
	{
